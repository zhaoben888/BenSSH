...
# ⚠️ 架构级修复：防右键漂移，强制锁定执行上下文到当前真实目录
...
if (-Not (Test-Path $sourceExe)) {
    Write-Host "❌ 错误: 找不到 benssh.exe ！" -ForegroundColor Red
    Write-Host "请确保你把本安装脚本和编译好的 benssh.exe 放在同一个文件夹下运行。" -ForegroundColor Yellow
    Pause
    exit
}
...
if (Test-Path $sourceIco) {
    Copy-Item $sourceIco -Destination "$installPath\benssh.ico" -Force
}
...
if ($oldPath -notmatch [regex]::Escape($installPath)) {
    ...
    Write-Host "✅ 环境变量注入完成！你现在可以在任意处输入 benssh 召唤本程序了。" -ForegroundColor Green
} else {
...
$Shortcut = $WshShell.CreateShortcut("$env:USERPROFILE\Desktop\BenSSH.lnk")
$Shortcut.TargetPath = "$installPath\$exeName"
$Shortcut.WorkingDirectory = "$installPath"
if (Test-Path "$installPath\benssh.ico") {
    $Shortcut.IconLocation = "$installPath\benssh.ico"
} else {
...
Write-Host "1. 你现在可以直接双击桌面上的 BenSSH 图标使用了！"
Write-Host "2. 如果你想在终端里使用，请【关闭当前所有黑框并重新打开】，然后直接输入 benssh 敲回车即可！"
Pause
