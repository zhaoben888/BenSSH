...
pub fn add_vps(entries: &mut Vec<VpsEntry>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let use_pwd = Confirm::new()
        .with_prompt("是否使用密码登录？(选 No 则配置私钥)")
        .default(true)
        .interact()?;

    let (password, key_path) = if use_pwd {
        let pwd = Password::new()
            .with_prompt("登录密码 (输入时不可见)")
            .interact()?;
        if pwd == "!q" {
            return Err("操作已取消".into());
        }
...
pub fn edit_vps(entries: &mut Vec<VpsEntry>, idx: usize) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let name: String = Input::new()
        ...
        .interact_text()?;
    if name == "!q" {
        return Err("编辑已取消，没有任何内容被修改".into());
    }
    ...
    if Confirm::new()
        .with_prompt("是否需要重新设置密码？")
        .default(false)
        .interact()?
    {
        let p = Password::new()
            .with_prompt("新密码 (输入时不可见)")
            .interact()?;
        if p == "!q" {
            return Err("编辑已取消，没有任何内容被修改".into());
        }
...
pub fn delete_vps(
    ...
    if Confirm::new()
        .with_prompt(format!("🚨 确定要永久删除节点 [{}] 吗？", name))
        .default(false)
        .interact()?
    {
        entries.remove(idx);
...
pub fn setup_ssh_key(
    entries: &mut Vec<VpsEntry>,
    idx: usize,
) -> Result<(), Box<dyn std::error::Error>> {
    println!("\n=== 🔑 开始为节点配置底层免密证书登录 ===");
    provision_ssh_key(entries, idx, true)?;
...
pub fn ensure_ssh_key(
    ...
    provision_ssh_key(entries, idx, false)
}

fn provision_ssh_key(
    entries: &mut Vec<VpsEntry>,
    idx: usize,
    verbose: bool,
) -> Result<(), Box<dyn std::error::Error>> {
