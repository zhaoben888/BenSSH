...
- 📂 **SFTP 极速文件流**：直接在终端浏览远程目录，文件上传下载极速响应。
...
- 🔑 **ED25519 免密注入**：一键后台生成高强度 ED25519 密钥对，自动注入远程 Linux 服务器 `authorized_keys`，并自动抹除本地明文密码。
- 🛡️ **智能认证回退**：完美兼容传统密码、ED25519 私钥，并针对部分安全策略严格的服务器提供了 `keyboard-interactive` 键盘交互式认证的智能回退与自动应答逻辑，彻底解决认证死锁问题。
...
- `e`     : 编辑当前节点
- `x`     : 永久删除该节点
- `s`     : 执行底层证书免密配置（自动生成 ED25519 密钥并下发）
- `q`     : 退出程序
